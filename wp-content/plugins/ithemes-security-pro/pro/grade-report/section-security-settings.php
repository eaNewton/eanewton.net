...
class ITSEC_Grading_System_Section_Security_Settings extends ITSEC_Grading_System_Section {
	...
	private function set_array_value_at_index( $array, $index, $value ) {
		...
		$child_array = set_array_value_at_index( $array[$current_index], $index, $value );
		...
		return array(
			...
			'backup' => array(
				...
				'scores' => array(
					...
					false => array(
						'percent'     => 0,
						'description' => __( 'Fix to enable Database Backups. This is highly recommended as backups can help recover the site in case an attack is successful.', 'it-l10n-ithemes-security-pro' ),
					),
				),
			),
			'brute-force' => array(
				...
				'scores' => array(
					...
					false => array(
						'percent'     => 0,
						'description' => __( 'Fix to enable Database Backups. This is highly recommended as it helps protect against attackers that use a large number of attacks to compromise site security.', 'it-l10n-ithemes-security-pro' ),
					),
				),
			),
			'magic-links' => array(
		...
		if ( 'custom' === $available_methods ) {
			...
			if ( 3 === count( $custom_available_methods ) ) {
				...
				if ( array( 'Two_Factor_Backup_Codes', 'Two_Factor_Totp' ) === $custom_available_methods ) {
					...
					return array(
						'percent'     => 85,
						'description' => __( 'Fix to change setting to All Methods as recommended. Currently the Backup Verification Codes method is disabled. This method is important as it provides a way for users to regain access to their account in the event that their device is damaged/lost or they lose access to email.', 'it-l10n-ithemes-security-pro' ),
					);
				}
				...
				return array(
					'percent'     => 0,
					'cap'         => '85',
					'description' => __( 'Fix to change setting to All Methods as recommended. Currently only the Backup Verification Codes method is available. This method is only useful as a fallback in the event that the Mobile App device is damaged/lost or email access is unavailable.', 'it-l10n-ithemes-security-pro' ),
				);
			} else if ( 'Two_Factor_Email' === $custom_available_methods[0] ) {
				return array(
					'percent'     => 60,
					'cap'         => '85',
					'description' => __( 'Fix to change setting to All Methods as recommended. Currently only the Email method is available. This is very limiting and could result in users not using two-factor due to lack of Mobile App support or frustration due to not having the Backup Verification Codes fallback option.', 'it-l10n-ithemes-security-pro' ),
				);
			} else if ( 'Two_Factor_Totp' === $custom_available_methods[0] ) {
				return array(
					'percent'     => 60,
					'cap'         => '85',
					'description' => __( 'Fix to change setting to All Methods as recommended. Currently only the Mobile App method is available. This may seem like a very strong setting, but taking away the Email method disables other valuable Two-Factor Authentication features offered by iThemes Security and taking away the Backup Verification Codes method makes logging in after loss or damage of the device very difficult.', 'it-l10n-ithemes-security-pro' ),
				);
			}
		}

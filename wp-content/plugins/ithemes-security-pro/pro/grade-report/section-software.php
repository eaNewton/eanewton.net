...
class ITSEC_Grading_System_Section_Software extends ITSEC_Grading_System_Section {
	...
	private function update_plugin( $slug ) {
		...
		if ( ! is_object( $update_plugins ) || ! isset( $update_plugins->response ) || ! isset( $update_plugins->response[$slug] ) ) {
		...
		$upgrader = new Plugin_Upgrader( new ITSEC_Upgrader_Skin() );
		$result = $upgrader->upgrade( $slug );

		if ( is_wp_error( $result ) ) {
			ITSEC_Response::add_error( new WP_Error( 'itsec-grading-system-plugin-update-failed', sprintf( __( 'Unable to update the %1$s plugin. %2$s', 'it-l10n-ithemes-security-pro' ), $name, $result->get_error_message() ) ) );
		} else if ( false === $result ) {
			ITSEC_Response::add_error( new WP_Error( 'itsec-grading-system-plugin-update-failed-unknown', sprintf( __( 'An unknown issue prevented the %s plugin from updating successfully. Please try again at a later time.', 'it-l10n-ithemes-security-pro' ), $name ) ) );
		} else {
			ITSEC_Response::add_message( sprintf( __( 'The %s plugin updated successfully.', 'it-l10n-ithemes-security-pro' ), $name ) );
		}
	}
	...
	public function get_criteria() {
		$criteria = array(
			'wordpress' => $this->get_wordpress_report( self::get_wordpress_version() ),

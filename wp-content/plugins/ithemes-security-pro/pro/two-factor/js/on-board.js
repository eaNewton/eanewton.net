	...
	var ScreenProviders = wp.Backbone.View.extend( {
		...
		prepare: function () {
			return {
				c: config,
				d: {
					disabled: this.canComplete() ? '' : 'disabled',
				}
			}
		},
		...
		prepare: function () {
			...
			return {
				...
				d: {
					...
					generateDisabled: this.state.get( 'backup-codes' ).isGenerating ? 'disabled' : '',
					continueDisabled: (this.state.get( 'backup-codes' ).isGenerating || this.state.get( 'backup-codes' ).warningVisible) ? 'disabled' : '',
				},
			}
		},

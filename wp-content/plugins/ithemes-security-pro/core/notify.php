...
class ITSEC_Notify {
	...
	public function notification_strings() {
		$description = esc_html__( 'During periods of heavy attack, iThemes Security can generate a LOT of email.', 'it-l10n-ithemes-security-pro' );
		...
		return array(
			'label'       => esc_html__( 'Security Digest', 'it-l10n-ithemes-security-pro' ),
			'description' => $description . ' ' . $features,
			'subject'     => esc_html__( 'Daily Security Digest', 'it-l10n-ithemes-security-pro' ), // Default schedule is Daily
		);
	}
	...
	public function send_daily_digest( $sent, $last_sent, $data ) {
		...
		switch ( $nc->get_schedule( 'digest' ) ) {
			case ITSEC_Notification_Center::S_DAILY:
				$title = esc_html__( 'Daily Security Digest', 'it-l10n-ithemes-security-pro' );
				$banner_title = sprintf( esc_html__( 'Your Daily Security Digest for %s', 'it-l10n-ithemes-security-pro' ), '<b>' . date_i18n( $df ) . '</b>' );
				break;
			case ITSEC_Notification_Center::S_WEEKLY:
				...
				$title = esc_html__( 'Weekly Security Digest', 'it-l10n-ithemes-security-pro' );
				$banner_title = sprintf( esc_html__( 'Your Weekly Security Digest for %s', 'it-l10n-ithemes-security-pro' ), '<b>' . $period . '</b>' );
				break;
			case ITSEC_Notification_Center::S_MONTHLY:
				...
				$title = esc_html__( 'Monthly Security Digest', 'it-l10n-ithemes-security-pro' );
				$banner_title = sprintf( esc_html__( 'Your Monthly Security Digest for %s', 'it-l10n-ithemes-security-pro' ), '<b>' . $period . '</b>' );
				break;
			default:
				$period = sprintf(
					'%s - %s',
				...
				$title = esc_html__( 'Security Digest', 'it-l10n-ithemes-security-pro' );
				$banner_title = sprintf( esc_html__( 'Your Security Digest for %s', 'it-l10n-ithemes-security-pro' ), '<b>' . $period . '</b>' );
				break;
		}
		...
		$mail->add_info_box( sprintf( esc_html__( 'The following is a summary of security related activity on your site: %s', 'it-l10n-ithemes-security-pro' ), '<b>' . $mail->get_display_url() . '</b>' ) );
		...
		$mail->add_details_box( sprintf(
			esc_html__( 'For more details, %1$svisit your security logs%2$s', 'it-l10n-ithemes-security-pro' ),
